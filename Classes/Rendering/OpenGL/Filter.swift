...
class Filter: FilterProtocol {
    ...
    func processPixelBuffer(_ pixelBuffer: CVPixelBuffer?, time: TimeInterval) -> CVPixelBuffer? {
        guard let shader = shader, let pixelBuffer = pixelBuffer, let textureCache = textureCache, let _ = outputFormatDescription, let bufferPool = bufferPool, let renderTextureCache = renderTextureCache else {
            return nil
        }
        let oldContext = EAGLContext.current()
        if oldContext !== glContext {
