        ...
        assetsHandler.mergeAssets(segments: segments, completion: { url in
            performUIUpdate {
                if let url = url {
                    ...
                    self.hideLoading()
                    let alertController = UIAlertController(title: nil, message: NSLocalizedString("Something goofed. Try again?", comment: "Alert controller message"), preferredStyle: .alert)
