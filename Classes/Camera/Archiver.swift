    ...
    func handle(export: EditorViewController.ExportResult) -> KanvasCameraMedia? {
        ...
        case (.image(let image), .image(let original)):
            if let url = image.save(info: export.info), let originalURL = original.save(info: export.info, in: self.saveDirectory) {
                ...
                let archiveURL = self.archive(media: export.original!, archive: export.archive, to: url.deletingPathExtension().lastPathComponent)
                return KanvasCameraMedia(image: image, url: url, original: originalURL, info: export.info, archive: archiveURL)
            } else {
                return nil
            }
...
//                    try! FileManager.default.moveItem(at: original, to: originalURL)
            ...
            return KanvasCameraMedia(asset: asset, original: original, info: export.info, archive: archiveURL)
        default:
            return nil
        }
    }
